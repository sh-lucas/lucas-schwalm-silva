...
export function App() {
  ...
  // Connection states
  const [streamStatus, setStreamStatus] = useState<'sse' | 'polling' | 'connecting'>('connecting')
  const [lastUpdated, setLastUpdated] = useState<Date>(new Date())
  const [updateFlash, setUpdateFlash] = useState(false)

  // Excuse Generator state
  const [productionExcuse, setProductionExcuse] = useState<string>("Click the button to pull a production-ready excuse out of thin air.")

  const rollExcuse = () => {
    const randomExcuse = PRODUCTION_EXCUSES[Math.floor(Math.random() * PRODUCTION_EXCUSES.length)]
  ...
  useEffect(() => {
    ...
    const connectSSE = () => {
      ...
      try {
        eventSource = new EventSource('https://checkup.sh-lucas.dev/api/metrics/stream')
        ...
        <div className="container">
          ...
          <header className="profile-header">
            <div className="avatar-wrapper">
              ...
              />
              <div className={`pulse-indicator ${streamStatus === 'sse' ? 'active' : 'inactive'}`} />
            </div>
            <div className="profile-info" style={{ gap: '1rem' }}>
              <div>
                <h1 className="title-gradient" style={{ fontSize: '2.8rem', lineHeight: '1.1' }}>
                  Hello, I am <span style={{ textDecoration: 'underline' }}>Lucas</span>!
                </h1>
                <p style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.85rem', marginTop: '0.4rem' }}>
              ...
              <div>
                <p style={{ color: 'var(--text-main)', fontSize: '1rem', lineHeight: '1.6' }}>
                  {age} y/o student at <span style={{ color: 'var(--accent-primary)', fontWeight: 600 }}>UERGS</span>, looking for experience and self development.
                </p>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.35rem' }}>
                  Currently coding production-ready bugs at{' '}
                  <a href="https://roxcode.io/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-cyan)', fontWeight: 500, textDecoration: 'none' }}>Roxcode</a>.
                </p>
              </div>

              <div className="social-links" style={{ marginTop: '0.25rem' }}>
                <a href="https://github.com/sh-lucas" target="_blank" rel="noopener noreferrer" className="social-btn outline">
                  <Github size={18} /> Github
                </a>
                <a href="https://www.linkedin.com/in/lucas-schwalm-silva/" target="_blank" rel="noopener noreferrer" className="social-btn outline">
                  <Linkedin size={18} /> LinkedIn
                </a>
                <a href="mailto:lucas.schwalm.silva@gmail.com" className="social-btn primary">
          ...
          <main className="info-content">
              ...
              <div className="glass-card fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <h3 style={{ fontSize: '1.1rem', color: 'var(--text-main)', fontWeight: 600 }}>Full Stack Engineer</h3>
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>2024 – present</span>
                  </div>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.15rem' }}>
                    <a href="https://roxcode.io/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-cyan)', textDecoration: 'none' }}>Roxcode</a> · Remote (Brazil)
                  </p>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.6rem', lineHeight: '1.55' }}>
                    API integrations, system design, and container deployments. Day-to-day stack: Go, MongoDB, TypeScript, RabbitMQ.
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.75rem' }}>
                    {['Go', 'MongoDB', 'TypeScript', 'RabbitMQ'].map(t => (
                      <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.05)', padding: '0.15rem 0.5rem', borderRadius: '4px', border: '1px solid var(--border-color)' }}>{t}</span>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <h3 style={{ fontSize: '1.1rem', color: 'var(--text-main)', fontWeight: 600 }}>Computer Engineering</h3>
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>2023 – 2027</span>
                  </div>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.15rem' }}>UERGS · Porto Alegre, Brazil</p>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.6rem', lineHeight: '1.55' }}>
                    Compilers, parallel computing, hardware interfaces, low-level scheduling. Strong math foundation.
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.75rem' }}>
                ...
                {/* Minor experiences */}
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem' }}>
                  <h4 style={{ fontSize: '0.75rem', color: 'var(--text-dark)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '1rem' }}>Minor experiences</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                    <div>
                      <p style={{ fontSize: '0.9rem', color: 'var(--text-main)', fontWeight: 500 }}>Dumbly self-hosting k3s</p>
                      <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '0.2rem', lineHeight: '1.5' }}>Running a personal Kubernetes cluster on Oracle Free Tier. Podman, NixOS, Cloudflare Tunnels.</p>
                    ...
                    <div>
                      <p style={{ fontSize: '0.9rem', color: 'var(--text-main)', fontWeight: 500 }}>checkup — server metrics API</p>
                      <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '0.2rem', lineHeight: '1.5' }}>Real-time system health endpoint powering this dashboard. SSE streaming, REST fallback.</p>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.5rem' }}>
                        {['Rust', 'SQLite'].map(t => (
                          <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.05)', padding: '0.15rem 0.5rem', borderRadius: '4px', border: '1px solid var(--border-color)' }}>{t}</span>
                        ))}
            ...
            {tab === 'Projects' && (
              <div className="glass-card fade-in project-list">
                {PROJECTS.map(p => (
                  <a key={p.name} href={p.link} target="_blank" rel="noopener noreferrer" className="project-item">
                    <div className="project-header">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <ExternalLink size={13} style={{ color: 'var(--text-dark)', flexShrink: 0 }} />
                      </div>
                    </div>
            ...
            {tab === 'Live Monitor' && (
              <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ padding: '0 0.25rem', marginBottom: '1.25rem' }}>
                  <h2 style={{ fontSize: '1.5rem', color: 'var(--text-main)', fontWeight: 600 }}>Live Node Monitor</h2>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginTop: '0.2rem' }}>
                    Personal k3s setup on Oracle Free Tier · Last sync: {lastUpdated.toLocaleTimeString()}
                  </p>
                </div>
                ...
                <div className="metrics-grid">
                  ...
                  />
                  <MetricCard
                    icon={<Activity className="metric-icon" style={{ color: 'var(--accent-cyan)' }} />}
                    label="Memory"
                  ...
                  />
                  <MetricCard
                    icon={<HardDrive className="metric-icon" style={{ color: 'var(--accent-emerald)' }} />}
                  ...
                  />
                  <MetricCard
                    icon={<Coffee className="metric-icon" style={{ color: 'var(--accent-amber)' }} />}
                ...
                <ExcuseGenerator excuse={productionExcuse} onRoll={rollExcuse} />
              </div>
            )}
          </main>
